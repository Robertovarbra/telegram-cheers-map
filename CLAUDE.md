...
Ruff config (`pyproject.toml`): target `py39`, line length **160**, rules `E,F,I,W,UP,N`, `web/` excluded. Match these when editing.

## Config / running

`src/config.py` loads env vars (via `python-dotenv` from `.env`, or real env vars which take priority and are preferred in production). Required: `BOT_TOKEN` (the app exits with `SystemExit` if missing). `BOT_USERNAME` is required for Mini App deep links to work — without it `/map` produces broken `t.me/{bot}/map?startapp=...` links. Other vars: `WEB_HOST` (default `0.0.0.0`), `WEB_PORT` (8080), `WEB_URL`, `INITDATA_MAX_AGE_HOURS` (default 24 — how long a Mini App `initData` signature stays valid). See `.env.example`.
...
`README.md` opens with a large numbered list (1–51) of known issues and improvements tagged 🔴 critical / 🟡 important / 🟢 nice-to-have, with ✅ marking done items. Consult it before starting work — it's the de-facto issue tracker and records intent behind existing code.
